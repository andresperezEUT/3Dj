...
//  private AccelerometerListener listener;
  ...
  Method orientationEventMethod;
  Method gravityEventMethod;
  ...
  public AccelerometerManager(Context parent) {
    ...
    try {
      orientationEventMethod =
        parent.getClass().getMethod("orientationEvent", new Class[] { Float.TYPE, Float.TYPE, Float.TYPE });
    } catch (Exception e) {
      // no such method, or an error.. which is fine, just ignore
    }
    ...
    try {
      gravityEventMethod =
        parent.getClass().getMethod("gravityEvent", new Class[] { Float.TYPE, Float.TYPE, Float.TYPE });
    } catch (Exception e) {
      // no such method, or an error.. which is fine, just ignore
    }
...
//    AccelerometerListener accelerometerListener = (AccelerometerListener) context;
    ...
    if (orientationSensors.size() > 0) {
      ...
      running = sensorManager.registerListener(sensorEventListener, orientationSensor, SensorManager.SENSOR_DELAY_FASTEST);
    }
    ...
    if (gravitySensors.size() > 0) {
      ...
      running = sensorManager.registerListener(sensorEventListener, gravitySensor, SensorManager.SENSOR_DELAY_FASTEST);
    }
    
    ////////////// REGISTER LINEAR ACCELERATION SENSOR ///////////////////////////////
    List<Sensor> accelerationSensors = sensorManager.getSensorList(Sensor.TYPE_LINEAR_ACCELERATION);
    if (accelerationSensors.size() > 0) {
      accelerationSensor = accelerationSensors.get(0);
      running = sensorManager.registerListener(sensorEventListener, accelerationSensor, SensorManager.SENSOR_DELAY_FASTEST);
    }
  }
      ...
      switch(sensor.getType()) {
        case Sensor.TYPE_LINEAR_ACCELERATION: 
          // System.out.println("acceleration");
            try {accelerationEventMethod.invoke(context, new Object[] { x, y, z });}
            catch (Exception e) {};
          break;
        case Sensor.TYPE_GRAVITY: 
          //System.out.println("gravity");
            try {gravityEventMethod.invoke(context, new Object[] { x, y, z });}
            catch (Exception e) {};
          break;
        case Sensor.TYPE_ORIENTATION:
         // System.out.println("orientation");
          try {orientationEventMethod.invoke(context, new Object[] { x, y, z });}
          catch (Exception e) {};
          break;
        }
